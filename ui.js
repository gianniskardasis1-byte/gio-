...
function handleMessage(msg) {
  switch (msg.type) {
    ...
    case "player_joined":
      players.push({ id: msg.id, name: msg.name, score: 0 });
...
function renderScorePanel() {
  ...
  for (const p of sorted) {
    ...
    const initial = (p.name || "?")[0].toUpperCase();
    div.innerHTML = `
      <div class="player-avatar" style="background:${avatarColor(p.id)}">${initial}</div>
      <span>${escHtml(p.name)}</span>
...
function showGameOver() {
  ...
  sorted.forEach((p, i) => {
    ...
    row.innerHTML = `
      ...
      <div class="player-avatar" style="background:${avatarColor(p.id)};width:28px;height:28px;font-size:.75rem">
...
function validateMenu() {
  ...
  const ip   = inputIp.value.trim() || "localhost";
  const port = inputPort.value.trim() || "5555";
  if (isNaN(parseInt(port, 10))) { setError("Port must be a number."); return null; }
  setError("");
  return { name, ip, port };
}
...
btnHost.addEventListener("click", () => {
  ...
  if (!v) return;
  // Show the lobby immediately; the server is started server-side.
  // For the web client we just connect to localhost.
  lobbyInfo.textContent = `Hosting on ${v.ip}:${v.port}  (share your IP with friends)`;
  connect(v.ip, v.port, v.name);
});

btnJoin.addEventListener("click", () => {
  const v = validateMenu();
  if (!v) return;
  lobbyInfo.textContent = `Connecting to ${v.ip}:${v.port}…`;
  connect(v.ip, v.port, v.name);
});
